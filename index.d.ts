 ...
 * @LastEditTime: 2020-08-14 22:45:15
...
import { DoTencentWebView } from "./ui/do_TencentWebView";
import { DoVerticalMarqueeLabel } from "./ui/do_VerticalMarqueeLabel";
import { DoVerticalSlideView } from "./ui/do_VerticalSlideView";
import { DoVideoRecord } from "./ui/do_VideoRecord";
import { DoVideoView } from "./ui/do_VideoView";
import { DoDoubleSlider } from "./ui/do_DoubleSlider";
import { Do3DCardAnimationView } from "./ui/do_3DCardAnimationView";
import { DoWebView1 } from "./ui/do_WebView1";

declare module "deviceone" {
  /** 这个是AppWork提供的基本应用类，通常在AppWork里开发的一个工程项目就是一个App实例。这个类负责页面Page的基本跳转，和App作用域内的数据共享等。 */
  export function sm(name: "do_App"): DoApp;
  /** 应用程序基本页面类，包括UI布局和逻辑代码。每一个移动应用都是由多个Page一层层的叠加来组成。
   * 在当前Page的基础上打开新的Page，新的Page打开后会盖住底部的Page，当新的Page关闭后，又会露出底部的Page。
...
注意：使用该组件的时候，需要在IDE的后台模式管理中加入“音频播放”这个设置 */
  ...
  export function sm(
    name: "do_FingerPrintRecognition"
  ): DoFingerPrintRecognition;
  ...
  export function ui(
    id: string,
    name: "do_FrameAnimationView"
  ): DoFrameAnimationView;
  ...
  export function ui(
    id: string,
    name: "do_TencentVodPlayerView"
  ): DoTencentVodPlayerView;
  ...
  export function ui(
    id: string,
    name: "do_ExpandableListView"
  ): DoExpandableListView;
  ...
  export function ui(
    id: string,
    name: "do_MultiSelectComboBox"
  ): DoMultiSelectComboBox;
  ...
  /** 集成了腾讯x5内核,可以加载html网页，拥有强大的视频播放功能；可以和原生进行交互。 */
  export function ui(id: string, name: "do_TencentWebView"): DoTencentWebView;
  /** 多条文字垂直滚动  */
  export function ui(
    id: string,
    name: "do_VerticalMarqueeLabel"
  ): DoVerticalMarqueeLabel;
  /** 这个UI组件包含多个子视图(UI文件），实现多个子视图之间上下平缓滑动效果，该组件还支持设置多个不同模板视图 */
  export function ui(
    id: string,
    name: "do_VerticalSlideView"
  ): DoVerticalSlideView;
  /** 录制视频，输出MP4格式 */
  export function ui(id: string, name: "do_VideoRecord"): DoVideoRecord;
  /** 一个简单的播放视频的UI组件，能支持播放本地和网络视频，可支持mp4格式，可以通过点击全屏按钮切换到全屏播放 */
  export function ui(id: string, name: "do_VideoView"): DoVideoView;
  /** 双滑块视图 */
  export function ui(id: string, name: "do_DoubleSlider"): DoDoubleSlider;
  /** 腾讯vod视图播放组件的精简版，支持腾讯业务和普通视频播放，同样支持缓存mp4和m3u8。适合自定义满屏播放业务！ */
  export function ui(
    id: string,
    name: "do_TencentSVPlayerView"
  ): DoTencentVodPlayerView;
  /** 纵向3D立体景深效果的卡片视图。手势交互，上下滑动切换卡片视图，带有较强的动画效果。支持设置网络图片和本地图片。 */
  export function ui(
    id: string,
    name: "do_3DCardAnimationView"
  ): Do3DCardAnimationView;
  /** 通常只能输入多行数据，当高度设置为-1时，默认会显示一行文字的高度 */
  export function ui(id: string, name: "do_WebView1"): DoWebView1;
}
